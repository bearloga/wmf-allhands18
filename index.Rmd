...
```{r comparing_distributions, fig.width=15, fig.height=5, out.width=624}
...
p3 <- ggplot(iris, aes(x = Sepal.Length, y = Species, fill = Species)) + 
  geom_density_ridges() +
...
The [density plot](https://en.wikipedia.org/wiki/Kernel_density_estimation) on the left is like a smooth histogram that doesn't discretize the variable into bins. The [violin plot](https://en.wikipedia.org/wiki/Violin_plot) in the middle is a rotated version that makes it easier to perform the comparison because the densities (distributions) are not overlapping. Recently, an alternative called *ridgeline plot* -- on the right -- gained a lot of popularity for comparing distributions across groups. (They were originally called *joyplots* but that name [was deprecated](http://serialmentor.com/blog/2017/9/15/goodbye-joyplots).)
